...
# okt에서 Mecab으로 변경. 여기에선 Mecab이 가장 정확하게 나온다.
...
def collate_fn(data):
    ...
    max_word_len = int(np.amax([len(word_tokens) for word_tokens in x_idx_batch])) 
...
def get_loader(data_file_dir, vocab, char_vocab, pos_vocab, lex_dict, batch_size, shuffle, num_workers, dataset='klp'):
    ...
    if dataset == 'klp':
        ...
        x_list, x_pos_list, x_split_list, y_list = load_data_and_labels_exo(data_file_dir='./data/EXOBRAIN_NE_CORPUS_10000.txt')
        y_list = np.array(y_list)
    elif dataset == 'both':
        x_list, x_pos_list, x_split_list, y_list = load_data_and_labels_klp(data_file_dir=data_file_dir)
        x_list_2, x_pos_list_2, x_split_list_2, y_list_2 = load_data_and_labels_exo(data_file_dir='./data/EXOBRAIN_NE_CORPUS_10000.txt')
