...
# Mecab으로 다시변경. 여기에선 Mecab이 가장 정확하게 나온다.
mecab = Mecab()
            ...
            if word_mor in re_word_list[0][0]:
                ...
                if B_flag == 0 and re_word_list[0][0].startswith(word_mor):
                    ...
                    if data_len == B_I_data_len:
                        ...
                        if x_mor[i + 1] not in re_word_list[0][0]:
                            y_data[i] = 'O'
                            B_flag = 0
                            data_len = 0
                            B_I_data_len = 0
...
def generate_word_embedding(x_list):
    ...
    wv_model_ko.save('./data/word2vec/ko_word2vec_' + str(args.word2vec_dim) + '.model')
...
def main(args):
    ...
    x_list_2, x_pos_list_2, x_split_list_2, y_list_2 = load_data_and_labels_exo('./data/EXOBRAIN_NE_CORPUS_10000.txt')
    ...
    lexicon_list, NER_double_list = load_lexicon_NER('./data/Gazetteer/korean_Gazetteer')
